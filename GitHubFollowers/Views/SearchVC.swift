...
extension SearchVC: NonoverlappingKeyboard {
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            ...
            let coverSize = usernameTf.frame.origin.y + usernameTf.frame.height - keyboardSize.minY // can also use keyboardSize.origin.y
            
            if coverSize > 0 {
                self.view.frame.origin.y -= coverSize
            }
        }
    }
